...
/*
* 建立此類別的目的是因為希望JPA在建立表格時生成的欄位可以依照Entity的field順序，故額外從Hibernate的部分內容取出，
* 以JAVA的機制會優先讀取這個類以取代Hibernate包的PropertyContainer
* 1.需要注意的是把TreeMap改成LikedHashMap會降低一點效能
* 2.更好的做法是自定義一個JPA依賴，但也有礙於時間成本以及維護性的問題
*/
